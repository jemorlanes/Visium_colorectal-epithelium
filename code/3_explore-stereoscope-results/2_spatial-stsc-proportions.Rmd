...
stsc.dir <- "/Users/javierescudero/Documents/work/Helmsley_wLudvigL/results/stsc_res/Colon_Alexandre/new_sc_Alexandre/HV_curated_variableGenes"
vis.dir <- "/Users/javierescudero/Documents/work/Helmsley_wLudvigL/results/Colon_Alexandre/stsc_cellType_visualization/regions_of_interest/HV-curated-varGenes"
```
...
We are only going to keep the PINs of interest, as this is mostly just to do some replotting
```{r}
pins <- c("1454","1732")
infoTable <- infoTable %>%
  filter(PIN %in% pins)
...
Once we have defined the regions of interest for these sections, we shall redo the plotting of the stereoscope proportions that Alexandre mentioned.
...
walk(unique(se.sub$PIN), function(sampleid){
  ...
  walk(types_interest, function(type){
    ...
    he_figure <- MapFeatures(se_sub, features = type, image_use = "raw",
      ...
      # labs(title = celltype, subtitle = paste(group, tissue, pin, sep = " : "),
      #      fill = "Celltype proportions") &
