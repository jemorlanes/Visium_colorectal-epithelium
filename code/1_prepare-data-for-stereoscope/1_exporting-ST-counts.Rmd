...
```{r}
rm(list=ls())
.libPaths("/home/javier.escudero/R/x86_64-pc-linux-gnu-library/4.0")
...
Setting directories
```{r}
st.dir <- "/home/javier.escudero/work/Helmsley_wLudvigL/data/Visium"
stsc.dir <- ""
```
...
Subsetting samples based on metadata of interest. In this case, we are going to focus on the ascending colon samples. In this group, we find samples taken from patients with Crohn's disease and/without IBS (Inflammatory Bowel Syndrome). My main idea is going to be to analyze separately all the samples from CD+/-IBS and then just take a look at what comes up.
```{r}
infoTable <- infoTable %>%
  filter(Group == "HV")
```
...
```{r}
ST.FeaturePlot(se.sub, features = "nFeature_RNA", label.by = c("sample_id"), pt.size = 3,
               ncol = 3, value.scale = "all", cols = viridis::magma(n = 11, direction = -1)) +
...
## Exporting Colon data
The colon data was exported using semla, so the script is in my local computer


Loading QC object
```{r}
se.cd <- readRDS(list.files("~/work/Helmsley_wLudvigL/data/Visium/Colon/data/curated", pattern = ".rds", full.names = TRUE))
```

Exporting counts
```{r}
exporting_counts(se.cd, unique(se.cd$sample_id), "~/work/Helmsley_wLudvigL/data/stsc/stsc_counts/CD")
```
