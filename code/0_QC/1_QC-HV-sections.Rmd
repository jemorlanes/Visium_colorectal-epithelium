...
# Summary
The goal of this markdown is to repeat the QC for the healthy volunteers sections. Initially we selected those spots with more than 150 unique genes, but this might be a bit too permisive. Let's clean that up a bit.
...
```
For now let's remove *V11D06-077_B1* and *V10B01-034_B1*, and set a quite mild threshold at 400 unique genes.
```{r}
...
Let's plot again. From looking at this plot, I can see that if we are more demanding with the QC, *V10B01-033_B1* ends up quite framented. However, some of the spots still look okay here. I am going to keep it for the analysis part, but being mindful that this section can throw things off down the line.
```{r, fig.asp = 1}
...
```{r}
saveRDS(se.manual,
        "/Users/javierescudero/Documents/work/Helmsley_wLudvigL/data/Visium/Colon/data/curated/se_clean_HV.rds")
```
